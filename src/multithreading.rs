...
const PARTICLE_COUNT: usize = 32768;
...
struct App {
    ...
    result_receiver: Option<mpsc::Receiver<Arc<PrimaryAutoCommandBuffer>>>,
...
impl App {
    ...
    fn init_threads(&mut self) {
        ...
        // Use concrete type for the channel
        let (result_tx, result_rx) = mpsc::channel::<Arc<PrimaryAutoCommandBuffer>>();

        // Use Arc + Mutex to share the single receiver across multiple threads
        // Standard mpsc::Receiver cannot be cloned.
        // A common pattern is Arc<Mutex<Receiver>> for multiple workers pulling from one queue.
        ...
        for _i in 0..THREAD_COUNT {
            ...
            let handle = thread::spawn(move || {
                // Each thread gets its own allocator to avoid lock contention
                // Wrap in Arc as required by AutoCommandBufferBuilder
                ...
                loop {
                    ...
                    let task = {
                        let lock = thread_task_rx.lock().unwrap();
                        lock.recv()
                    };
                    ...
                    match task {
                        Ok(task) => {
                            let mut builder = AutoCommandBufferBuilder::primary(
                                allocator.clone(),
                                task.queue_family_index,
                                CommandBufferUsage::OneTimeSubmit,
                            )
                            .unwrap();

                            // Divide global count (8192) by local group size (256)
                            // We need to calculate how many groups this specific thread task covers
                            let dispatch_x = task.push_constants.count / 256;

                            unsafe {
                                builder
                                    .bind_pipeline_compute(task.pipeline.clone())
                                    .unwrap()
                                    .bind_descriptor_sets(
                                        PipelineBindPoint::Compute,
                                        task.pipeline.layout().clone(),
                                        0,
                                        task.descriptor_set.clone(),
                                    )
                                    .unwrap()
                                    .push_constants(
                                        task.pipeline.layout().clone(),
                            ...
                            // Send Arc<PrimaryAutoCommandBuffer>
                            thread_result_tx.send(command_buffer).unwrap();
                        }
                        Err(_) => {
    ...
    fn create_compute_pipeline(&mut self) {
        ...
        let bytes = std::fs::read("assets/shaders/multithreading.spv")
            .expect("failed to read multithreading.spv");
    ...
    fn draw_frame(&mut self) {
        ...
        if let Some(mut fence) = self.fences[self.frame_index].take() {
            if fence.queue().is_some() {
                match fence.then_signal_fence_and_flush() {
                    Ok(f) => {
                        f.wait(None).unwrap(); // Wait for CPU fence
        ...
        let now = Instant::now();
        let delta_time = now.duration_since(self.last_time).as_secs_f32() * 2000.0;
        self.last_time = now;
        ...
        // --- Multithreaded Compute Recording ---

        let particles_per_thread = PARTICLE_COUNT as u32 / THREAD_COUNT as u32;
        let descriptor_set = self.compute_descriptor_sets[self.frame_index].clone();

        // Dispatch tasks
        if let Some(sender) = &self.task_sender {
            for i in 0..THREAD_COUNT {
                let task = ComputeTask {
                    pipeline: self.compute_pipeline.as_ref().unwrap().clone(),
                    descriptor_set: descriptor_set.clone(),
                    push_constants: PushConstants {
                        delta_time,
                        start_index: i as u32 * particles_per_thread,
                        count: particles_per_thread,
                    },
        ...
        // Collect results
        let mut compute_command_buffers: Vec<Arc<PrimaryAutoCommandBuffer>> =
            Vec::with_capacity(THREAD_COUNT);
        if let Some(receiver) = &self.result_receiver {
            for _ in 0..THREAD_COUNT {
                // This blocks until a thread finishes recording
                let buffer = receiver.recv().unwrap();
                compute_command_buffers.push(buffer);
            }
        }

        // Graphics Command Buffer (Main Thread)
        ...
        // Sync & Submit
        // Sequence: Initial Future -> Compute CB 1 -> ... -> Compute CB N -> Graphics CB -> Present

        let mut future = self.fences[self.frame_index]
            .take()
            .unwrap_or_else(|| vulkano::sync::now(self.device.as_ref().unwrap().clone()).boxed())
            .join(acquire_future)
            .boxed();

        // Chain all compute buffers
        for buf in compute_command_buffers {
            future = future.then_execute(queue.clone(), buf).unwrap().boxed();
        }

        // Chain graphics
        let final_future = future
